...
public sealed class LighthouseCapabilityRouter(JsonUtil jsonUtil) : StaticRouter(jsonUtil,
[
    new RouteAction<EmptyRequestData>("/manimal/lighthouse/capability",
        (_, _, _, _, _) => new ValueTask<string>(ReadCapability()))
])
{
    ...
    private static string ReadCapability()
    {
        ...
        if (File.Exists(path))
        {
            ...
            foreach (var file in manifest.ServerFiles) ManifestRules.VerifyFile(root, file.Path, file.Sha256);
            capability.ContentId = manifest.ContentId;
            capability.ManifestSha256 = ManifestRules.Hash(path);
            capability.Mode = manifest.Mode;
            capability.Ready = (manifest.Ready && manifest.Mode == "probe" && File.Exists(Path.Combine(root, "allow-probe"))) ||
                (LighthouseServerState.Allows(manifest, root) && LighthouseServerState.AppliedManifestHash == capability.ManifestSha256);
        }
