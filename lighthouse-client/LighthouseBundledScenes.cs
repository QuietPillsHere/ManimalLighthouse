...
{
    internal static bool BeforeLoad(ResourceKey? resourceKey, LoadSceneMode loadSceneMode, bool allowSceneActivation,
        Action<float> progressCallback, ref LoadSceneOperation __result)
    {
        if (resourceKey == null || !LighthouseSceneLoader.Owns(Path.GetFileNameWithoutExtension(resourceKey.path)))
        {
            return true;
        }
    ...
    private sealed class BundledSceneOperation : LoadSceneOperation
    {
        internal async Task Begin(string path, LoadSceneMode mode, bool activate, Action<float> progress)
        {
            ...
            try
            {
                ...
                {
                    throw new InvalidDataException("Bundled scene could not load: " + path);
                }
                ...
                while (!AsyncOperation.isDone)
                {
                    ...
                    if (!activate && AsyncOperation.progress >= 0.9f)
                    {
                        break;
                    }
