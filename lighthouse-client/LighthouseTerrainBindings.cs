...
{
    // Also compiled unchanged into the Unity graphics probe.
    internal static class LighthouseTerrainBindings
    {
        // Populated by Unity's field serializer from the verified bundle.
...
#pragma warning restore CS0649
        ...
        private static readonly Dictionary<string, ResourcesForProfile> Profiles = new Dictionary<string, ResourcesForProfile>();
        private static readonly Dictionary<Terrain, string> Bound = new Dictionary<Terrain, string>();
        ...
        public static void Load(AssetBundle bundle)
        {
            ...
            if (!json) throw new InvalidOperationException("Lighthouse rendering binding table is missing.");
            ...
            if (!_basemap || !_basemap.isSupported) throw new InvalidOperationException("Lighthouse terrain basemap shader is unsupported.");
            foreach (var name in _settings.Terrains) Names.Add(name);
        }

        public static bool Restore(Terrain terrain)
        {
            if (!_bundle || !terrain || !Names.Contains(terrain.name)) return false;
            string key = MicroSplatObject.currentSeason + "/" + MicroSplatObject.currentQuality;
            if (Bound.TryGetValue(terrain, out var previous) && previous == key) return false;
            ...
            if (!Profiles.TryGetValue(key, out var resources))
            {
                ...
                foreach (var profile in _settings.Profiles)
                    if (profile.Season + "/" + profile.Quality == key) { selected = profile; break; }
                if (selected == null) throw new InvalidOperationException("Missing terrain season/quality profile: " + key);
            ...
            if (!component && active) terrain.gameObject.SetActive(false);
            try
            {
                if (!component) component = terrain.gameObject.AddComponent<MicroSplatTerrain>();
                ...
                component.Sync();
                if (!terrain.materialTemplate || !terrain.materialTemplate.GetTexture("_Control0"))
                    throw new InvalidOperationException("Terrain material synchronization failed: " + terrain.name);
                Bound[terrain] = key;
            }
            finally { if (active && !terrain.gameObject.activeSelf) terrain.gameObject.SetActive(true); }
            return true;
        }
        ...
        public static void Validate()
        {
            ...
            foreach (var terrain in Bound.Keys) if (terrain) restored.Add(terrain.name);
            if (restored.Count != Names.Count) throw new InvalidOperationException("Restored " + restored.Count + " of " + Names.Count + " Lighthouse terrain tiles.");
        }

        public static void Refresh()
        {
            var terrains = new List<Terrain>(Bound.Keys);
            foreach (var terrain in terrains) if (terrain) Restore(terrain);
        }
        ...
        public static void Clear()
        {
            ...
            {
                if (resources.Properties.propTex) UnityEngine.Object.Destroy(resources.Properties.propTex);
                UnityEngine.Object.Destroy(resources.Properties);
                UnityEngine.Object.Destroy(resources.Keywords);
            }
            Profiles.Clear(); Bound.Clear(); Names.Clear(); _bundle = null; _basemap = null; _settings = null;
        }
    }
}
