...
public static class ManifestRules
{
    ...
    private static void ValidateFiles(List<PayloadFile> files)
    {
        ...
        {
            if (file == null || !paths.Add(file.Path)) throw new InvalidDataException("Duplicate payload file.");
    ...
    {
        if (hash is not { Length: 64 }) throw new InvalidDataException("SHA-256 is required for every payload.");
